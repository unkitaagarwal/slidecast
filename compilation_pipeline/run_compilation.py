...
def run_one_compilation(theme: str,
                        brand_card_path: str = None,
                        brand_name: str = None) -> str:
    """Generate one full compilation carousel.

    `brand_card_path` — when set, replaces the bundled Slidecast CTA image on
    slide 06 with the user's uploaded brand image.
    `brand_name` — when set, substitutes any "Slidecast" mention in the
    Gemini-generated CTA caption lines with the user's brand name so the
    final slide reads "Like > Share > <user_brand>" instead.
    """
    print(f"\n=== {theme} ===")
    if brand_card_path:
        print(f"  brand override: {brand_card_path}")
    if brand_name:
    ...
    # Mid-carousel CTA — apply optional brand overrides
    cta_lines = comp.cta_caption[:]
    if brand_name:
        cta_lines = [line.replace("Slidecast", brand_name) for line in cta_lines]
    composite_cta(cta_lines,
                  os.path.join(slides_dir, "06_cta.png"),
                  card_path=brand_card_path)
