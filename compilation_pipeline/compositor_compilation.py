...
def composite_recipe_page(recipe: dict, out_path: str,
    ...
    # no text can collide with the SLIDECAST footer.
    FOOTER_RESERVE = 140
    ...
    # ---- Bottom decorative band + Slidecast watermark ----
    ...
    wm = "SLIDECAST  /  meal planner & grocery list"
...
def composite_cta(cta_lines: list[str], out_path: str,
                  card_path: Optional[str] = None) -> str:
    ...
    `card_path` lets callers override the App-Store CTA card image — used
    when a paid user has uploaded their own brand image. Falls back to the
    bundled `assets/cta_card.png` if the override is missing or unset.
    """
    # Resolve the brand card to render. The default ships with Slidecast.
    effective_card = card_path if (card_path and os.path.exists(card_path)) else CTA_CARD_PATH
    ...
    card = None
    card_h = 0
    if os.path.exists(effective_card):
        card = Image.open(effective_card).convert("RGBA")
